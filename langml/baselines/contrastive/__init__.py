...
class Evaluator:
    ...
    def compute_corrcoef(self, data: List[Tuple[str, str, int]]) -> float:
        ...
        left_token_ids = keras.preprocessing.sequence.pad_sequences(left_token_ids, padding='post', truncating='post')
        right_token_ids = keras.preprocessing.sequence.pad_sequences(right_token_ids, padding='post', truncating='post')
